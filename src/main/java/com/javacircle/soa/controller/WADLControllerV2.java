	...
	public @ResponseBody Application generateWadl(HttpServletRequest request) {
		...
		for (Map.Entry<RequestMappingInfo, HandlerMethod> entry : handletMethods.entrySet()) {
			...
			for (RequestMethod httpMethod : httpMethods) {
				...
				rep.setMediaType("application/json");

				Annotation[][] annotations = javaMethod.getParameterAnnotations();
				Class<?>[] paramTypes = javaMethod.getParameterTypes();
				Parameter[] parameter = javaMethod.getParameters();
				int i = 0;
				for (Annotation[] annotation : annotations) {
					Class<?> paramType = paramTypes[i];
					Parameter paramObj = parameter[i];
					i++;
					for (Annotation annotation2 : annotation) {
						...
						if (annotation2 instanceof RequestBody) {
							...
							waldParam.setType(new QName(paramType.getTypeName()));
						...
						//	wadlRequest.getParam().add(waldParam);
							wadlRequest.getRepresentation().add(rep);
							//wadlResponse.getRepresentation().add(rep);
